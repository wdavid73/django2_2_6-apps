...
def list_view(request):
    obj_coti_clien = CotizacionClient.objects.all().filter(state=1)
...
def restore_view(request):
    obj_coti_clien = CotizacionClient.objects.all().filter(state=0)
    context = {
        "cotizacion_client": obj_coti_clien
    }
...
def restore(request):
    ...
    obj = get_object_or_404(CotizacionClient, id=id)
    if request.method == 'POST':
        obj.state = 1
        obj.save()
        return redirect('coti_client_list')
